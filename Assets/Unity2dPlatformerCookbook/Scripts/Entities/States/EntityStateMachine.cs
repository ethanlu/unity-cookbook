...
namespace Unity2dPlatformerCookbook.Scripts.Entities.States
{
    public class EntityStateMachine
    {
        ...
        private IEntityState GetState(string state)
        {
            ...
            {   // lazy load state
                var state_class = Type.GetType($"Unity2dPlatformerCookbook.Scripts.Entities.States.{state}State, Assembly-CSharp");
                if (state_class is null)
                {
                    throw new Exception($"Could not load assembly : Unity2dPlatformerCookbook.Scripts.Entities.States.{state}State, Assembly-CSharp");
                }
                ...
                if (s is null)
                {
                    throw new Exception($"Could not instantiate class : Unity2dPlatformerCookbook.Scripts.Entities.States.{state}State");
                }
                _states[state] = s;
            }
